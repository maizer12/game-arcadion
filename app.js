...
let game = {
	...
	preload(callback) {
		...
		this.preloadSprite(onResourceLoad)
		this.preloadAudio(onResourceLoad)
	},
	preloadSprite(onResourceLoad) {
	...
	render() {
		...
		this.renderBlocks()
		this.ctx.fillText('Очки: ' + this.score, 30, 20)
		this.ctx.fillText('Час: ' + this.time, this.width / 2, 20)
		this.ctx.fillText('Уровень: ' + this.lvl, this.width - 100, 20)
	},
	renderBlocks() {
		for (let block of this.blocks) {
			if (block.active) {
...
game.ball = {
	...
	collideWorldBounds() {
		...
		if (ballLeft < worldLeft) {
			...
			this.dy = this.velocity
			game.sounds.bump.play()
		} else if (ballBottom > worldBottom || game.time <= 0) {
			game.end('Game over')
		}
	},
	bumpBlock(block) {
